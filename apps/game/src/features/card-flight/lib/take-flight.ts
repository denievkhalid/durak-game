...
import { BOT_PLAYER_ID, HUMAN_PLAYER_ID } from "@durakjs/engine"
...
export function buildTakeFlights(
  view: GameViewDTO,
  handSlotStart: number,
  takerId: string = BOT_PLAYER_ID,
): PendingFlight[] {
  const flights: PendingFlight[] = []
  let slotIndex = handSlotStart
  const faceDown = takerId !== HUMAN_PLAYER_ID
